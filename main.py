...
def page4():
    # bar = char1()
    bar, fig1, fig2, fig3, fig4, histo = tansuat_tso_tstl_page4()
    return render_template('page4.html', plot=bar, tanso=fig1, tansuat=fig2, tstl=fig3, box=fig4,  hist=histo)


@app.route('/page5')
def page5():
    # bar = char1()
    bar, fig1, fig2, fig3, fig4, histo = tansuat_tso_tstl_page5()
    return render_template('page4.html', plot=bar, tanso=fig1, tansuat=fig2, tstl=fig3, box=fig4,  hist=histo)
...
def tansuat_tso_tstl_page5():
    ...
    figTanSo.add_trace(go.Bar(x=x_Trade, y=tanso_Trade, text=tanso_Trade,
    ...
    figTanSuat.add_trace(go.Pie(labels=x_Trade, values=tanXuat_Trade))

    tanXuatTL_Trade = []
    s_Trade = 0
    for i in tanXuat_Trade:
        s_Trade += i
        tanXuatTL_Trade.append(s_Trade)
    figTstl = go.Figure()
    figTstl.add_trace(go.Scatter(x=x_Trade, y=tanXuatTL_Trade,
    ...
    return graphJSON, graphJSON1, graphJSON2, graphJSON3, graphBox, histo
# ---------PAGE4---------
...
def tansuat_tso_tstl_page4():
    ...
    for i in value_Gross:
        if i < min(value_Gross)+h_Gross-0.01:
            khoang1_Gross.append(i)
        elif i >= min(value_Gross)+h_Gross and i < min(value_Gross)+2*h_Gross-0.01:
            khoang2_Gross.append(i)
        elif i >= min(value_Gross)+2*h_Gross and i < min(value_Gross)+3*h_Gross-0.01:
            khoang3_Gross.append(i)
        elif i >= min(value_Gross)+3*h_Gross:
            khoang4_Gross.append(i)
    figTanSo = go.Figure()
    x_Gross = []
    x_Gross.append('0 - '+str(k1_Gross-0.01))
    x_Gross.append(str(round(k1_Gross, 2)) + " - " +
                   str(round(k2_Gross, 2)-0.01))
    x_Gross.append(str(round(k2_Gross, 2)) + " - " +
                   str(round(k3_Gross, 2)-0.01))
    x_Gross.append(str(round(k3_Gross, 2))+" - " +
                   str(round(max(value_Gross), 2)))

    tanso_Gross = [len(khoang1_Gross), len(khoang2_Gross),
                   len(khoang3_Gross), len(khoang4_Gross)]

    figTanSo.add_trace(go.Bar(x=x_Gross, y=tanso_Gross, text=tanso_Gross,
                              textposition='inside', marker=dict(color='#339966')))
    ...
    figTanSuat = go.Figure()
    figTanSuat.add_trace(go.Pie(labels=x_Gross, values=tanXuat_Gross))
    ...
    figTstl.add_trace(go.Scatter(
        x=x_Gross, y=tanXuatTL_Gross, marker=dict(color='#339966')))
    figTstl.update_layout(
        xaxis_title='USD',
        yaxis_title='Tần xuất tích lũy')
